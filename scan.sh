...
CRIT_COUNT=$(jq '[.matches // [] | .[] | select(.vulnerability.severity == "Critical")] | length' "vulns.cyclonedx.json")
HIGH_COUNT=$(jq '[.matches // [] | .[] | select(.vulnerability.severity == "High")] | length' "vulns.cyclonedx.json")
MED_COUNT=$(jq '[.matches // [] | .[] | select(.vulnerability.severity == "Medium")] | length' "vulns.cyclonedx.json")
LOW_COUNT=$(jq '[.matches // [] | .[] | select(.vulnerability.severity == "Low")] | length' "vulns.cyclonedx.json")
UNKNOWN_COUNT=$(jq '[.matches // [] | .[] | select(.vulnerability.severity == "Unknown")] | length' "vulns.cyclonedx.json")
