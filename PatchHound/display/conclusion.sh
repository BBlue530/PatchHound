...
if [ "$FAIL_ON_CRITICAL" = "true" ] && [ "$CRITICAL_COUNT_SAST" -gt 0 ]; then
  print_message "[!]" "Critical vulnerability found" "Failing due to $CRITICAL_COUNT_SAST critical issues found by SAST scan."
  FAIL="true"
fi

if [ "$FAIL_ON_CRITICAL" = "true" ] && [ "$CRIT_COUNT_TRIVY" -gt 0 ]; then
  print_message "[!]" "Critical vulnerability found" "Failing due $CRIT_COUNT_TRIVY critical vulnerabilities found by Trivy scan."
  FAIL="true"
fi

if [ "$FAIL_ON_CRITICAL" = "true" ] && [ "$MISCONF_COUNT_TRIVY" -gt 0 ]; then
  print_message "[!]" "Critical vulnerability found" "Failing due $MISCONF_COUNT_TRIVY misconfigurations found by Trivy scan."
  FAIL="true"
fi

if [ "$FAIL_ON_CRITICAL" = "true" ] && [ "$SECRET_COUNT_TRIVY" -gt 0 ]; then
  print_message "[!]" "Critical vulnerability found" "Failing due $SECRET_COUNT_TRIVY secrets found by Trivy scan."
  FAIL="true"
fi
