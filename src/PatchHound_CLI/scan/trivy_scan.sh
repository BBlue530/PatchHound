if [ "$TRIVY_SCAN" = "true" ]; then
  if [ "$SCAN_IMAGE" = "true" ]; then
    ...
    "$BASE_DIR_BIN/trivy" image --scanners "$TRIVY_IMAGE_SCANNERS" --format json -o trivy_report.json "$TARGET"
  else
    print_message "[~]" "Running Trivy scan on source repo..." ""
    "$BASE_DIR_BIN/trivy" fs --scanners "$TRIVY_FS_SCANNERS" --format json -o trivy_report.json "$TARGET"
  fi
else
