...
def log_exporter_config(app_config):
    ...
    if sum([https_log_exporter_enabled, opentelemetry_log_exporter_enabled]) > 1:
        print("[!] More than 1 scan data storage is enabled: [backend.storage.scheduled_rescan.]")
        sys.exit(1)
    
    if https_log_exporter_enabled:
        os.environ["https_log_exporter_enabled"] = "True"
        
        os.environ["https_log_export_url"] = expand_env(app_config.get("backend", {}).get("export_log", {}).get("https", {}).get("export_url", None))
        os.environ["https_log_export_url_api_key"] = expand_env(app_config.get("backend", {}).get("export_log", {}).get("https", {}).get("export_url_api_key", None))
        ...
        os.environ["OTEL_EXPORTER_OTLP_LOGS_ENDPOINT"] = expand_env(app_config.get("backend", {}).get("export_log", {}).get("opentelemetry", {}).get("export_url", None))
        
        opentelemetry_log_export_url_api_key = expand_env(app_config.get("backend", {}).get("export_log", {}).get("opentelemetry", {}).get("export_url_api_key", None))
        ...
        os.environ["SERVICE_NAME"] = expand_env(app_config.get("backend", {}).get("export_log", {}).get("opentelemetry", {}).get("service_name", None))
        os.environ["ENVIRONMENT"] = expand_env(app_config.get("backend", {}).get("export_log", {}).get("opentelemetry", {}).get("environment", None))
