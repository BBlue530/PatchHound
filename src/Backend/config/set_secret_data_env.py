...
def secret_data_storage_config(app_config):
    generate_local_secrets_enabled = app_config.get("backend", {}).get("storage", {}).get("secret_data", {}).get("local", {}).get("generate_secrets", {}).get("enabled", False)

    local_secrets_enabled = app_config.get("backend", {}).get("storage", {}).get("secret_data", {}).get("local", {}).get("secrets", {}).get("enabled", False)
    ...
    if secret_manager_enabled:
        ...
        os.environ["api_key"] = app_config.get("backend", {}).get("storage", {}).get("secret_data", {}).get("local", {}).get("secrets", {}).get("secrets_name", {}).get("api_key", None)
        os.environ["jwt_key"] = app_config.get("backend", {}).get("storage", {}).get("secret_data", {}).get("local", {}).get("secrets", {}).get("secrets_name", {}).get("jwt_key", None)
        os.environ["cosign_key"] = app_config.get("backend", {}).get("storage", {}).get("secret_data", {}).get("local", {}).get("secrets", {}).get("secrets_name", {}).get("cosign_key", None)
    else:
...
def set_external_secrets_in_env(app_config):
    if os.environ.get("secret_in_env_enabled", "False").lower() == "true":
        secret_name = app_config.get("backend", {}).get("storage", {}).get("secret_data", {}).get("secret_manager", {}).get("aws", {}).get("secret_manager_name", None)
